...
class WhisperMic:
    def __init__(self,model="base",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=False,energy=300,pause=1,dynamic_energy=False,save_file=False, model_root="~/.cache/whisper",mic_index=None):
    ...
    def __setup_mic(self, mic_index):
        ...
        try:
            with self.source as mic:
                self.recorder.adjust_for_ambient_noise(mic, duration=0.2)
        except sr.WaitTimeoutError:
            self.logger.warning("Timeout: No speech detected within the specified time during ambient noise adjustment.")
        except sr.UnknownValueError:
            self.logger.warning("Speech recognition could not understand audio during ambient noise adjustment.")
